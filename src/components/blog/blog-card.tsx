    ...
    <Link href={`/blog/${post.slug}`}>
      <article className='group relative overflow-hidden rounded-lg border border-gray-200 bg-white transition-all hover:shadow-md dark:border-gray-700 dark:bg-gray-800'>
        ...
        <div className='relative p-4'>
          ...
          <div className='mb-2'>
            <span
              className={`inline-block rounded-full bg-gradient-to-r ${gradient} px-2.5 py-0.5 text-xs font-medium text-white`}
            >
              {categoryLabel}
            </span>
          </div>
          ...
          {/* Title */}
          <h2 className='mb-2 text-base font-semibold leading-snug text-gray-900 transition-colors group-hover:text-opacity-80 dark:text-white'>
            {post.title}
          </h2>
